...
Return ONLY valid JSON with this structure:
{
  ...
  "categories": [
    ...
    result.categories.forEach(cat => {
        console.log(`  ${cat.category_name}: ${cat.movies.length} movies`);
    });
    
    const totalMovies = result.categories.reduce((sum, cat) => sum + cat.movies.length, 0);
    console.log(`\n  TOTAL: ${totalMovies} movies\n`);
}

singleFetch().catch(err => console.error('Error:', err.message));
